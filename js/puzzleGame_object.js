...
/************* 变量 ******************/	
	...
	this.imgRandRotate=[];//图片打乱顺序后排序的角度。
	this.imgOrigRotate=[];//图片拆分后，存储正确角度
...
/* 代码整理 */
	...
	selectImage:function () {
		var self=this;
		$("#change_image").bind('change',function () {
			file=this;
	...
	gameState:function(){
		...
		this.btnStart.bind('mousedown',function(){
			...
			if(self.hasStart == 0){//不在游戏中
				...
				//打乱图片
				self.randomArr();
				self.cellOrder(self.imgRandArr ,self.imgRandRotate);
				...
				self.imgCells.css({
					...
					console.log(cellIndex_1);
					var rotate=(self.imgRandRotate[cellIndex_1]+90)%360;
					self.imgRandRotate[cellIndex_1]=rotate;
					/*if(rotate==360){
						self.imgRandRotate[cellIndex_1]=0;
					}*/
					$(this).css("transform","rotate("+rotate+"deg)");
					if(self.checkPass(self.imgOrigArr,self.imgRandArr) && self.checkPass(self.imgRandRotate,self.imgOrigRotate)){
						self.success();
					}
				}).bind('mouseup',function(){
					clearTimeout(timeFunName);
				...
				//复原图片
				self.cellOrder(self.imgOrigArr ,self.imgOrigRotate);
				//取消事件绑定
				self.imgCells.css('cursor','default').unbind('dblclick').unbind('mouseover').unbind('mouseout').unbind('mousedown');
			}
		});		
	},
	...
	/**
	 * [randomRotate 生成旋转随机角度]
	 * @return [无]
	 */
	randomRotate:function () {
		imgCells=this.imgCells;
		this.imgRandRotate=[];
		for(var i=0;i<imgCells.length;i++){
			this.imgRandRotate[i]=Math.floor(Math.random()*3)*90;
		//	;
		}
		return;
	},
	...
	cellOrder:function(arr,rotate){
		...
		//		"transform":"rotate("+rotate[i]+"deg)",
			...
			this.imgCells.eq(i).css({"transform":"rotate("+rotate[arr[i]]+"deg)"});
		}
	},
	...
	cellExchange:function(from,to){
		...
		this.imgCells.eq(to).css('z-index','30').animate({
			...
			//两块图片交角度数据。
			/*var tempRotate=self.imgRandRotate[to];
            self.imgRandRotate[to]= self.imgRandRotate[from];
			self.imgRandRotate[from]=tempRotate;*/
			//判断是否完成全部移动，可以结束游戏
			if(self.checkPass(self.imgOrigArr,self.imgRandArr) && self.checkPass(self.imgRandRotate,self.imgOrigRotate)){
				self.success();
			}
	...
	success:function(){
		...
		this.imgCells.unbind('dblclick').unbind('mousedown').unbind('mouseover').unbind('mouseout');
...
/* 加入图片，运行代码 */
$(function(){
	var image={'img':'images/man.jpg'};
	var pg = new puzzleGame(image);
